...
void MAIN::InitScene() {
	...
	m_pTestTitleScene = new TitleScene;
...
void MAIN::Loop()
{
	...
	BaseScene::eScene NowScene = BaseScene::Title;
	...
	while (msg.message != WM_QUIT)//終了メッセージが届いていなければ　ループ
	{
		if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) //メッセージをもとに処理を選択
		{
			...
			m_pFPS->Run();
			//fpsを一定に保ちたいってなるとどうしたらいいんだろう
			//if (m_pFPS->FPSLimit()) {
				if (NowScene == BaseScene::Title) {
					NowScene = m_pTestTitleScene->App();
					if (NowScene != BaseScene::Title) {
						m_pTestTitleScene->Release();
					}
				}
				if (NowScene == BaseScene::Game) {
					NowScene = m_pTestGameScene->App();
					if (NowScene != BaseScene::Game) {
						m_pTestGameScene->Release();
					}
				}
				if (NowScene == BaseScene::Result) {
					NowScene = m_pResultScene->App();
					if (NowScene != BaseScene::Result) {
						m_pResultScene->Release();
					}
				}
				if (NowScene == BaseScene::End) {
					PostQuitMessage(0);
				}
			//}
		}
