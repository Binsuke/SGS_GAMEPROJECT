...
void MoveUIPanel::Init(ID3D11Device* inDevice, ID3D11DeviceContext* inDeviceContext)
{
	m_pDevice = inDevice;
	m_pDeviceContext = inDeviceContext;
}

//void MoveUIPanel::Release()
//{
//	SAFE_RELEASE(m_pVertexBuffer);
//	SAFE_RELEASE(m_pSampleLinear);
//	SAFE_RELEASE(m_pTexture);
...
HRESULT MoveUIPanel::CreateVertexBuffer() {

	float size = MyPoly::Poly::PolyHalfSize * 0.5;
	SimpleVertex vertices[] =
	{
		D3DXVECTOR3(-size,-size,0.0f),D3DXVECTOR2(0,1),//頂点１
		D3DXVECTOR3(-size,size,0.0f),D3DXVECTOR2(0,0),//頂点２
		D3DXVECTOR3(size,-size,0.0f),D3DXVECTOR2(1,1),//頂点３
		D3DXVECTOR3(size,size,0.0f),D3DXVECTOR2(1,0),//頂点４
	};
...
HRESULT MoveUIPanel::CreateTexture(std::string filename)
{
	//テクスチャー用サンプラー作成
