...
public:	
	...
	bool SetMapData(int Index, const D3DXVECTOR3& vPos, int Size);

	//何かが範囲内にいたらトゥルー
	bool EnemyIsNone(float Leftx, float Rightx, float forwardz, float backwardz,int thisindex);
	...
	bool CanMoveRight(int index, int numcube);

	bool CanMoveLeft(int index, int numcube);

	bool CanMoveForward(int index, int numcube);

	bool CanMoveBackward(int index, int numcube);
...
private:
	...
	//データがセットされた位置を保存する用の構造体
	struct SetDataPosition{
		int dataposx;
		int dataposz;
	};

	std::vector<SetDataPosition> MapInfoPosIndex;

	std::queue<SetDataPosition> ClearPosQue;
