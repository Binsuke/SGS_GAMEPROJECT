...
HRESULT MAIN::InitModel()
{
	...
	bd.ByteWidth = sizeof(SimpleVertex) * 3;//サイズが3になったため数を変更
...
void MAIN::Render()
{
	...
	//プリミティブトポロジーをセット
	m_pDeviceContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);//線にするために変更

	//プリミティブをレンダリング
	m_pDeviceContext->Draw(3, 0);//バーテックスが3つになったからここも変更
