...
void GameScene::InitModel()
{
	...
	for (int i = 0; i < m_iNumEnemy; i++) {
		...
		/*if (i < EnemyNumLV1) {
			m_pEnemyModel[i]->InitLv(1);
			float Size = m_pEnemyModel[i]->GetSize();
			m_pEnemyModel[i]->SetPos(0, 0, Size*(i)*2);
		}
		else if (i - EnemyNumLV1 < EnemyNumLV2)
		{
			m_pEnemyModel[i]->InitLv(2);
			float Size = m_pEnemyModel[i]->GetSize();
			float z = (Size*(i - EnemyNumLV1)) * 2;
			m_pEnemyModel[i]->SetPos(1, 0, z);
		}
		else if (i - EnemyNumLV1 - EnemyNumLV2 < EnemyNumLV3)
		{
			m_pEnemyModel[i]->InitLv(3);
			float Size = m_pEnemyModel[i]->GetSize();
			float z = (Size*(i - EnemyNumLV1 - EnemyNumLV2)) * 2;
			m_pEnemyModel[i]->SetPos(2 + Size * 2, 0, z);
		}
		else if (i - EnemyNumLV1 - EnemyNumLV2 - EnemyNumLV3 < EnemyNumLV4)
		{
			m_pEnemyModel[i]->InitLv(4);
			float Size = m_pEnemyModel[i]->GetSize();
			float z = (Size*(i - EnemyNumLV1 - EnemyNumLV2 - EnemyNumLV3)) * 2;
			m_pEnemyModel[i]->SetPos(3 + Size * 2, 0, z);
		}
		else {
			m_pEnemyModel[i]->InitLv(5);
			float Size = m_pEnemyModel[i]->GetSize();
			float z = (Size*(i - EnemyNumLV1 - EnemyNumLV2 - EnemyNumLV3 - EnemyNumLV4)) * 2;
			m_pEnemyModel[i]->SetPos(4 + Size * 2, 0, z);
		}*/
		/*m_pEnemyModel[i]->InitLv(1);
		float Size = (m_pEnemyModel[i]->GetSize());
		if (i % 3 == 0) {
