...
void MAIN::Render()
{
	...
	if (DeltaFPS->DeltaReady())
	{
		...
		if (m_eMoveVec == eMoveVec::Right) {
			...
			bool check = m_pTestModel->AnimationLocalRight(DeltaFPS->GetDeltaTime());

			//check = m_pEnemyModel[0]->AnimationLocalRight(DeltaFPS->GetDeltaTime());
			if (check) {
...
void MAIN::MoveUpdate(eMoveVec eVec)
{
	...
	case eMoveVec::Forward:
		...
		m_Camera->GetMoveLocalVecA(&vLocalVec, Camera::eGetForward);
		m_pTestModel->Move(m_FPS->GetDeltaTime(), vLocalVec);
		break;
	case eMoveVec::Backward:
		//D3DXVECTOR3 vLocalBackwardVec(0,0,0);
		m_Camera->GetMoveLocalVecA(&vLocalVec, Camera::eGetBackward);
		m_pTestModel->Move(m_FPS->GetDeltaTime(), vLocalVec);
		break;
	case eMoveVec::Right:
		//D3DXVECTOR3 vLocalRightVec(0,0,0);
		m_Camera->GetMoveLocalVecA(&vLocalVec, Camera::eGetRight);
		m_pTestModel->Move(m_FPS->GetDeltaTime(), vLocalVec);
		break;
	case Left:
		D3DXVECTOR3 vLocalLeftVec;
		m_Camera->GetMoveLocalVecA(&vLocalLeftVec, Camera::eGetLeft);
