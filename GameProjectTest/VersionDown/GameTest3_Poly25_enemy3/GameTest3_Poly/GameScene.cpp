...
BaseScene::eScene GameScene::App()
{
	...
	//桁落ちなのでは？と考えて試してみる
	float LeftX = m_pTestModel->GetPos().x - m_pTestModel->GetSize() * 2;
	float RightX = m_pTestModel->GetPos().x + m_pTestModel->GetSize() * 2;
	float ForwardZ = m_pTestModel->GetPos().z + m_pTestModel->GetSize() * 2;
	float BackwardZ = m_pTestModel->GetPos().z - m_pTestModel->GetSize() * 2;
