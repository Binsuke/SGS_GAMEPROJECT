...
void MAIN::Render()
{
	...
	if (DeltaFPS->DeltaReady())
	{
		if (GetKeyState('A') & 0x80) {
			//MainCamera->MoveLeft(DeltaFPS->GetDeltaTime());
			//m_pTestModel->MoveWorldLeft(DeltaFPS->GetDeltaTime());
			D3DXVECTOR3 vLocalLeftVec;
			MainCamera->GetMoveLocalLeftVec(&vLocalLeftVec);
			m_pTestModel->Move(DeltaFPS->GetDeltaTime(), vLocalLeftVec);
		}
		if (GetKeyState('D') & 0x80) {
			//MainCamera->MoveRight(DeltaFPS->GetDeltaTime());
			//m_pTestModel->MoveWorldRight(DeltaFPS->GetDeltaTime());
			D3DXVECTOR3 vLocalRightVec;
			MainCamera->GetMoveLocalRightVec(&vLocalRightVec);
			m_pTestModel->Move(DeltaFPS->GetDeltaTime(), vLocalRightVec);
		}
		if (GetKeyState('W') & 0x80) {
			//MainCamera->MoveForward(DeltaFPS->GetDeltaTime());
			//m_pTestModel->MoveWorldForward(DeltaFPS->GetDeltaTime());
			D3DXVECTOR3 vLocalForwardVec;
			MainCamera->GetMoveLocalForwardVec(&vLocalForwardVec);
			m_pTestModel->Move(DeltaFPS->GetDeltaTime(), vLocalForwardVec);
		}
		if (GetKeyState('S') & 0x80) {
			//MainCamera->MoveBackward(DeltaFPS->GetDeltaTime());
			//m_pTestModel->MoveWorldBackward(DeltaFPS->GetDeltaTime());
			D3DXVECTOR3 vLocalBackwardVec;
			MainCamera->GetMoveLocalBackwardVec(&vLocalBackwardVec);
			m_pTestModel->Move(DeltaFPS->GetDeltaTime(), vLocalBackwardVec);
		}
