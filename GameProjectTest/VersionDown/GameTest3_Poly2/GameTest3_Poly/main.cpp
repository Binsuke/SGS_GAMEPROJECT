...
void MAIN::Render()
{
	...
	if(GetKeyState('A') & 0x80) {
		MainCamera->m_Pos -= 0.0025f * MainCamera->m_vRight;
	}
	if (GetKeyState('D') & 0x80) {
		MainCamera->m_Pos += 0.0025f * MainCamera->m_vRight;
	}
	if (GetKeyState('W') & 0x80) {
		MainCamera->m_Pos += 0.0025f * MainCamera->m_vForward;
	}
	...
	//D3DXMatrixRotationY(&MainCamera->m_Transform, MainCamera->m_Rot.y);
	...
	D3DXMatrixLookAtLH(&mView, &MainCamera->m_vEyePt, &MainCamera->m_vLookat, &MainCamera->m_vUp); //getting view matrix
