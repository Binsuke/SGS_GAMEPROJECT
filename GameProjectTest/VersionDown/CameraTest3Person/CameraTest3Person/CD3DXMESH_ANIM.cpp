...
HRESULT MY_HIERARCHY::CreateFrame(LPCSTR Name, LPD3DXFRAME *ppNewFrame)
{
	...
	*ppNewFrame = pFrame;//今まで設定してきたもの（初期化）を引数のポインターに格納している
	...
	LPD3DXMESHCONTAINER *ppMeshContainer)
{
	...
	*ppMeshContainer = NULL;
	...
	if (pMeshContainer == NULL)//確保できているかどうか
	{
		return E_OUTOFMEMORY;
	}
	...
	if (!(pMesh->GetFVF() & D3DFVF_NORMAL))
	{
		...
		hr = pMesh->CloneMeshFVF(pMesh->GetOptions(),
			...
			pDevice, &pMeshContainer->MeshData.pMesh);//MeshContainerのpMeshにメッシュデータをコピーしている？
		if (FAILED(hr))
		{
			return E_FAIL;
		}
		pMesh = pMeshContainer->MeshData.pMesh;
		D3DXComputeNormals(pMesh, NULL);//法線を作成している？
	...
	else
	{
		...
		pMeshContainer->MeshData.Type = D3DXMESHTYPE_MESH;
		pMesh->AddRef();//参照カウントを＋している
	}

	pMeshContainer->NumMaterials = max(1, NumMaterials);//マテリアルが１個なのかそれ以上なのか
	pMeshContainer->pMaterials = new D3DXMATERIAL[pMeshContainer->NumMaterials];//マテリアルの数だけマテリアルの動的確保
	pMeshContainer->ppTextures = new LPDIRECT3DTEXTURE9[pMeshContainer->NumMaterials];//マテリアルの数だけテクスチャーを確保
	pMeshContainer->pAdjacency = new DWORD[iFacesAmount * 3];//一面が三個頂点を使うから＊３？

	if ((pMeshContainer->pAdjacency == NULL) || (pMeshContainer->pMaterials == NULL))
	{
		return E_OUTOFMEMORY;//確保できてなかったら
	}
	...
	{
		pMeshContainer->pMaterials[0].pTextureFilename = NULL;//テクスチャがなかった場合
	...
	*ppMeshContainer = pMeshContainer;//ここまで設定してきたものを引数で受け取ったものに格納
...
//D3DXのパーサーを使うためにはDx9のデバイスが必要なので作成する
...
HRESULT CD3DXMESH_ANIM::LoadXAnimMesh(LPSTR FileName)
{
	...
	//この時点でファイルから取り出した全フレームがm_pFrameRootに入っている
	//またアニメーションをコントロールするにはm_pAnimControllerが初期化されてるはずなので、それを使う

	//あとは、そこから必要な情報を取り出しつつ各フレームごとに独自のメッシュを構築していく
...
{
	if (pFrame && pFrame->pMeshContainer)//同じだったらってことかな
										//この方法ならビットを見比べての比較になるから＝＝演算子をオーバーライドする必要がないってことかな？
...
{
	//この時点でファイルから取り出したメッシュデータがDx9のD3DXメッシュに入っている
	D3D11_BUFFER_DESC bd;
	D3D11_SUBRESOURCE_DATA InitData;

	MYFRAME* pFrame = (MYFRAME*)p;

	LPD3DXMESH pD3DXMesh = pFrame->pMeshContainer->MeshData.pMesh;//D3DXメッシュ
	PARTS_MESH* pAppMesh = new PARTS_MESH; //アプリメッシュに先ほどのデータをコピーする
	...
	for (DWORD i = 0; i < pAppMesh->dwNumMaterial; i++)
	{
		...
		//インデックスバッファーを作成
		//それに先立ち、メッシュの属性情報を得る　属性情報でインデックスバッファーから細かいマテリアルごとのインデックスバッファーを分離できる
		...
		//2バイトのインデックスの場合もありえる
		DWORD* pIndex = (DWORD*)new DWORD[pD3DXMesh->GetNumFaces() * 3];//面の数＊３個確保
		...
		for (DWORD i = 0; i < NumAttr; i++)
		{
			...
			bd.ByteWidth = sizeof(DWORD)* pAttrTable[i].FaceCount * 3;//DWORDサイズのフェイス＊３のサイズ
	...
	//D3DXMESHの場合、ロックしないとD3DXバーテックスバッファーから取り出せない
	...
	{
		//Dx9のD3DMESHのバーテックスバッファからの情報で、Dx11のアプリ定義バーテックスバッファを作成
		bd.Usage = D3D11_USAGE_DEFAULT;
...
void CD3DXMESH_ANIM::Render(D3DXMATRIX& mView, D3DXMATRIX& mProj, D3DXVECTOR3& vLight, D3DXVECTOR3& vEye)
{
	...
	D3DXVec3TransformCoord(&m_AxisX, &D3DXVECTOR3(1, 0, 0), &Rot);//x軸の作成かな？
...
void CD3DXMESH_ANIM::DrawPartsMesh(PARTS_MESH* pPartsMesh, D3DXMATRIX World)
{
	...
	if (SUCCEEDED(m_pDeviceContext11->Map(m_pConstantBuffer0, 0, D3D11_MAP_WRITE_DISCARD, 0, &pData)))
	{
		...
		//ワールド行列を渡す
		sg.mW = mWorld;
		D3DXMatrixTranspose(&sg.mW, &sg.mW);//転置を行っている
	...
	for (DWORD i = 0; i < pPartsMesh->dwNumMaterial; i++)
	{
		...
		if (SUCCEEDED(m_pDeviceContext11->Map(m_pConstantBuffer1, 0, D3D11_MAP_WRITE_DISCARD, 0, &pData)))
		{
			...
			sg.vAmbient = pPartsMesh->pMaterial[i].Ambient;//アンビエント
			sg.vDiffuse = pPartsMesh->pMaterial[i].Diffuse;//ディフューズ
			sg.vSpecular = pPartsMesh->pMaterial[i].Specular;//スペキュラー
