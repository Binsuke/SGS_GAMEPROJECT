...
HRESULT MAIN::InitModel()
{
	...
	{
		D3DXVECTOR3(-0.2f,0.7f,0.0f),	//線に変更するためポイントを追加

		D3DXVECTOR3(0.5f,0.5f,0.0f),
	};
	D3D11_BUFFER_DESC bd; //説明
	bd.Usage = D3D11_USAGE_DEFAULT;//使用法
	bd.ByteWidth = sizeof(SimpleVertex) * 2;//サイズが２になったため数を変更
...
void MAIN::Render()
{
	...
	//プリミティブトポロジーをセット
	m_pDeviceContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_LINELIST);//線にするために変更

	//プリミティブをレンダリング
	m_pDeviceContext->Draw(2, 0);//バーテックスが二つになったからここも変更
